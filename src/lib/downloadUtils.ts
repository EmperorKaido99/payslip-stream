...
const generateEncryptedPdf = async (
  ...
  page.drawText('ENCRYPTED PAYSLIP', {
    x: 50,
    y: height - 80,
    size: 28,
    font: helveticaBold,
  ...
  page.drawText('🔒 Password Protected with Employee ID', {
    x: 50,
    y: height - 110,
    size: 12,
    font: helvetica,
  ...
  const details = [
    { label: 'Name:', value: employeeName },
    { label: 'Employee ID:', value: employeeId },
    { label: 'Password:', value: `${employeeId} (Employee ID Number)` },
    { label: 'Page Number:', value: `${pageNumber}` },
    { label: 'Generated:', value: new Date().toLocaleDateString() },
    { label: 'File:', value: fileName },
  ];
  ...
  page.drawText(`This PDF is encrypted with password: ${employeeId}`, {
    x: 60,
    y: height - 400,
    size: 10,
    font: helvetica,
  ...
  // Footer
  page.drawText('This is a simulated encrypted payslip generated by PaySync for demonstration purposes.', {
    x: 50,
    y: 50,
    size: 9,
    font: helvetica,
  ...
  // In a real implementation, you would use a library like pdf-lib-encrypt or 
  // handle encryption server-side. For this demo, we're simulating the concept.
  return pdfDoc.save();
};
